...
TEST_CASE("align_sensors: correctly identifies which sensor maps to which phase", "[package]")
{
  // Sensor whose read() value is set externally so the test controls what
  // align_sensors sees for each excitation phase.
  struct ScriptedSensor : public ICurrentSensor
  {
    float value = 0.f;
    bool init_sensor() override { return true; }
    float read() override { return value; }
  ...
  ScriptedSensor s0, s1;
  MockDriver drv;

  // align_sensors reads each sensor once per excitation phase (A, B, C).
  // We intercept sleep_ to advance sensor values at the right moment.
  // Excitation sequence: drive A -> read -> drive 0 -> read, drive B -> read -> ...
  // The reads that matter are the ones taken while a phase is energised.
  // sleep_ is called 5 times: after A on, after A off, after B on, after B off, after C on.
  // Sensor reads happen at calls 1, 3, 5 (0-indexed: 0, 2, 4).
  int sleep_call = 0;
  auto sleep_fn = [&](int) {
    // Before the first read (after phase A energised): s0 sees A, s1 sees nothing
    if (sleep_call == 0) { s0.value =  5.f; s1.value = 0.f; }
    // After phase A de-energised: zero out
    if (sleep_call == 1) { s0.value =  0.f; s1.value = 0.f; }
    // Before the second read (after phase B energised): s1 sees B, s0 sees nothing
    if (sleep_call == 2) { s0.value =  0.f; s1.value = 5.f; }
    // After phase B de-energised: zero out
    if (sleep_call == 3) { s0.value =  0.f; s1.value = 0.f; }
    // Before the third read (after phase C energised): neither responds
    if (sleep_call == 4) { s0.value =  0.f; s1.value = 0.f; }
    ++sleep_call;
  };

  CurrentSensorPackage<2> pkg{{{&s0, &s1}}, sleep_fn};
  bool ok = pkg.align_sensors(drv, 0.5f);
  REQUIRE(ok);

  // Now read at steady state: s0=2, s1=3
  s0.value = 2.f;
  s1.value = 3.f;
  auto v = pkg.get_phase_currents(false);
  REQUIRE_THAT(v.a, WithinAbs(2.f,  tol));   // s0 -> phase A
  REQUIRE_THAT(v.b, WithinAbs(3.f,  tol));   // s1 -> phase B
  REQUIRE_THAT(v.c, WithinAbs(-5.f, tol));   // Kirchhoff
}
